...
class DocLSTM(nn.Module):
    ...
    def forward(self, body):
        ...
        head= self.proj(rsent.view(1,768))
...
class Similarity(nn.Module):
    def __init__(self, mem_dim, hidden_dim, num_classes, domain_feature):
        ...
        if self.domain_feature : # contact deep feature + domain feature
            self.wh = nn.Linear(((8*self.mem_dim)+8), self.hidden_dim) # for combined model
    ...
    def forward(self,head,body,feature_vec):
        ...
        if self.domain_feature: #for combined feature model
            ...
            concat_vec = torch.cat( (entail , torch.FloatTensor(feature_vec).reshape(1,len(feature_vec)) ), dim=1) # len(feature_vec)
...
class SimilarityTreeLSTM(nn.Module):
    def __init__(self,  in_dim, mem_dim, hidden_dim, sparsity, freeze, num_classes, \
        max_num_para, max_num_sent, max_num_word, domain_feature,feature_dim):
    ...
    def forward(self, body,  feature_vec = None):
        head, News_encoding  = self.doclstm(body)
        output = self.similarity(head.view(1, 2*self.mem_dim),News_encoding.view(1, 2*self.mem_dim),feature_vec )
        return output
