...
def preprocess_data(base_ip_dir, parsed_dir, info_dir, dataset_name, fname, data_type = ''):
    ...
    df['headline'].to_csv(fname_out_total,  index=None)

    # with open(fname_out_total,'r') as contents:
    #   save = contents.read()
    # with open(fname_out_total,'w') as contents:
    #       contents.write('headline\n')
    # with open(fname_out_total,'a') as contents:
    #       contents.write(save)
    # line_pre_adder(fname_out_total,'headline')



    fname_out_total = os.path.join(parse_dir_total,'label.txt')
    df['label'].to_csv(fname_out_total, index=None)
